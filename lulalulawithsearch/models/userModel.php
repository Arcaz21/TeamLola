...
//Billing
                function billsK1(){
                    $query = "SELECT student.LRN,fName, lName,(level.levelName) as level_name, (s_account.misc_balance) as misc, (s_account.tuition_balance) as tuition, (s_account.entrance_balance) as entrance from student
                        JOIN s_account ON student.LRN = s_account.LRN
                        JOIN level ON student.levelID = level.levelID
                        WHERE level.levelID = 11";
                    $result = mysqli_query($this->conn, $query);
                    $res = array();
                    while ($row = mysqli_fetch_array($result)){
                        array_push($res, $row);
                ...
                function payment($trans,$lrn){
                    $query = "UPDATE s_account 
                      SET 
                        misc_balance = \"".$trans['misc']."\",
                        tuition_balance = \"".$trans['tuition']."\",
                        entrance_balance = \"".$trans['entrance']."\",
                        total = \"".$trans['total']."\"
                        WHERE 
                        LRN = $lrn";       
                ...
                function addIn($in){
                    $query=" INSERT INTO invoice (invoice_num, misc_pay, tuition_pay, entrance_pay, total_pay, due, cashier, LRN ) 
                    VALUES (\"".$in['invoice_num']."\",\"".$in['misc_pay']."\", \"".$in['tuition_pay']."\", \"".$in['entrance_pay']."\", \"".$in['total_pay']."\", \"".$in['due']."\" , \"".$in['cashier']."\", \"".$in['LRN']."\")";
                ...
                function InvoiceInfo($lrn){
                    $query = "SELECT *  FROM invoice JOIN student on invoice.LRN = student.LRN JOIN s_account on invoice.LRN = s_account.LRN
                              WHERE invoice_num = \"".$lrn."\"
                              ";
