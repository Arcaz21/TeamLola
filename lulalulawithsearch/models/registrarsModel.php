...
from student WHERE levelID=11";
                ...
                function viewSinfo($LRN){
                    $query = "SELECT student.LRN, student.fName, student.lName, student.levelID, (select level.levelName from level where level.levelID=student.levelID) as level, student.levelID, student.age, student.mtongue, student.ipGroup, student.birthday, student.address, student.mother, student.father, (select guardian.fName from guardian where guardian.guardianID = student.guardianID) as guardianf, (select guardian.lName from guardian where guardian.guardianID = student.guardianID) as guardianl, (select guardian.contact from guardian where guardian.guardianID = student.guardianID) as contact, guardian.guardianID, syid FROM student join guardian on student.guardianID = guardian.guardianID join level on level.levelID = student.levelID join records on records.LRN = student.LRN
						WHERE student.LRN = \"".$LRN."\" ";
                    $result = mysqli_query($this->conn, $query);
...
/*    function addnewUsercheck($newU){
    ...
    function search($se){
                    $query = "SELECT * from student WHERE LRN LIKE '%".$se['search']."%' OR fName LIKE '%".$se['search']."%' OR lName LIKE '%".$se['search']."%' ";
                    $result = mysqli_query($this->conn, $query);
                    $res = array();
